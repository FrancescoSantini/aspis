...
FONT_REGULAR = "ASPISVera"
FONT_BOLD = "ASPISVera-Bold"
...
pdfmetrics.registerFontFamily("ASPISVera", normal=FONT_REGULAR, bold=FONT_BOLD)
TEMP_VECTOR_PDFS: set[Path] = set()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--layer-key", default="", help="Render one canonical project evidence layer")
    parser.add_argument("--layer-title", default="", help="Human-readable evidence-layer title")
...
def vector_plot_flowables(
    ...
    elif suffix == ".svg" and svg2rlg is not None and PdfReader is not None:
        try:
            ...
            renderPDF.drawToFile(drawing, str(temporary_pdf))
            TEMP_VECTOR_PDFS.add(temporary_pdf)
            plot = VectorPdfFlowable(temporary_pdf, vector_pdf_placements)
        except Exception:
            plot = None
        source_label = "source SVG converted to vector PDF"
    if plot is None:
        return []
    plot.hAlign = "CENTER"
    return [
...
def cleanup_temporary_vector_pdfs() -> None:
    for path in list(TEMP_VECTOR_PDFS):
        path.unlink(missing_ok=True)
        TEMP_VECTOR_PDFS.discard(path)


def append_remaining_multipage_assets(output: Path, assets: list[dict[str, str]]) -> int:
    """Append pages after page one from multi-page plot PDFs already embedded once."""
    ...
    for row in assets:
        ...
        if row.get("asset_kind", "") != "plot" or path.suffix.lower() != ".pdf" or not path.exists() or path in seen:
            continue
        seen.add(path)
        try:
            additions.extend(PdfReader(str(path)).pages[1:])
        except Exception:
            continue
    if not additions:
        return 0
...
def project_report_map_table(styles: dict[str, ParagraphStyle]) -> Table:
    rows = [
        ...
        ("RNA-seq", "Canonical differential-expression, enrichment, DTU/splicing, and isoform-switch evidence layers."),
        ("smallRNA", "Canonical miRNA differential-expression, target, feature-set, and matched-evidence layers."),
...
def render_project_report(args: argparse.Namespace) -> int:
    ...
    if not args.layer_key:
        raw_assets = infer_rnaseq_layout_assets(raw_assets, args.rnaseq_asset_manifest, project)
    ...
    story: list = []
    report_title = f"ASPIS {args.layer_title} Technical Report" if args.layer_key else "ASPIS Project Technical Report"
    story.append(para(report_title, styles["title"]))
    ...
    story.append(
        para(
            (
                f"This PDF is the single-file export for the {args.layer_title} evidence layer. It is organized by "
                "contrast and includes the layer's principal plots and short table excerpts."
                if args.layer_key
                else "This is the primary email-friendly single-file export for the project. It combines the "
                "canonical evidence-layer reports in project-report order."
            ),
            styles["body"],
        )
    )
    ...
    cleanup_temporary_vector_pdfs()
    if args.layer_key:
        appended = append_remaining_multipage_assets(output, assets)
        if appended:
            counter.pages += appended
            log_step(f"appended {appended} remaining page(s) from multi-page layer assets")

    done = Path(args.done)
    done.parent.mkdir(parents=True, exist_ok=True)
    with done.open("w", encoding="utf-8") as handle:
        handle.write("status\tproject\trnaseq_rows\tsmallrna_rows\tassets\tpages\n")
        final_status = row_status(rows) if rows else ("ok" if assets else "not_present")
        handle.write(
            f"{final_status}\t{project}\t{len(rnaseq_rows)}\t{len(smallrna_rows)}\t"
