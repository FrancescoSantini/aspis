...
def write_config_fragment(path: Path, target_table: Path, provenance_table: Path, summary_table: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
...
def main() -> int:
    ...
    if args.config_fragment:
        write_config_fragment(Path(args.config_fragment), target_table, provenance_table, summary_table)
