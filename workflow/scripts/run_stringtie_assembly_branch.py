#!/usr/bin/env python3
"""Run per-sample StringTie transcript assembly for an RNA-seq branch."""
...
REQUIRED_PLAN_COLUMNS = {"project", "assay", "status", "annotation_gtf", "transcriptome_mode"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--outdir", required=True, help="StringTie assembly output directory")
    parser.add_argument("--manifest", required=True, help="Assembly manifest TSV")
    ...
    parser.add_argument("--extra-args", default="", help="Extra StringTie assembly args")
...
def read_plan(path: Path) -> dict[str, str]:
    ...
    if row.get("transcriptome_mode") != "reference_guided_novel":
        raise ValueError(
            "StringTie assembly requires transcriptome_mode='reference_guided_novel', "
            f"got {row.get('transcriptome_mode')!r}"
        )
...
def validate_samples(rows: list[dict[str, str]], plan: dict[str, str]) -> None:
    ...
    if errors:
        raise ValueError("StringTie assembly cannot start:\n- " + "\n- ".join(errors))
...
def run_stringtie(
    ...
    log = sample_dir / "stringtie_assembly.log"
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"{row['library_id']}: StringTie exited with status {completed.returncode}")
    if not gtf.exists() or gtf.stat().st_size == 0:
        raise RuntimeError(f"{row['library_id']}: StringTie produced an empty GTF: {gtf}")
...
def main() -> int:
    ...
    write_manifest(Path(args.manifest), assembly_rows)
    write_done(Path(args.done), assembly_rows)
    return 0
