...
def inventory_row(
    *,
    ...
    if not report_id:
        suffix = ":".join(part for part in [assay, contrast_id, report_type, label.lower().replace(" ", "_")] if part)
        report_id = f"project:{project}:{suffix}" if project else f"run:{suffix}"
    if not parent_report_id and report_id != "run":
        parent_report_id = f"project:{project}" if project else "run"
    if not scope:
        scope = "contrast" if contrast_id else ("assay" if assay else ("project" if project else "run"))
    return {
        "report_id": report_id,
        ...
        "canonical_html": path_text(html_path) if html_path else "",
...
def build_report_inventory(
    ...
    for project in projects:
        ...
        for order, (layer_key, layer_label) in enumerate(
            [
            ...
            layer_dir = base_dir / "projects" / project / "layers" / layer_key
            rows.append(
                inventory_row(
                    report_type="evidence_layer",
                    label=layer_label,
                    project=project,
                    html_path=layer_dir / "index.html",
                    pdf_path=layer_dir / "technical_report.pdf",
                    summary_paths=[layer_dir / "technical_report.qa.tsv"],
                    manifest_paths=[base_dir / "projects" / project / "layers" / "layer_manifest.tsv"],
                    expected=False,
                    report_id=f"project:{project}:layer:{layer_key}",
                    parent_report_id=f"project:{project}",
    ...
    rows = [row for row in rows if row["report_type"] not in {"differential", "enrichment", "isoform_switch", "targets"}]
    for row in rows:
        if not row["contrast_id"]:
            continue
        if row["report_type"] == "contrast_enrichment":
            layer_key = "enrichment"
        elif row["report_type"] == "contrast_targets":
            layer_key = "mirna_targets"
        elif row["report_type"] == "contrast_summary" and row["assay"] == "smallrna":
            layer_key = "smallrna_de"
        else:
            layer_key = "rnaseq_de"
        row["parent_report_id"] = f"project:{row['project']}:layer:{layer_key}"
        row["navigation_level"] = "leaf"
        row["scope"] = "contrast"
    return rows
