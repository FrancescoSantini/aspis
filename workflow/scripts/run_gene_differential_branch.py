...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-count", type=int, default=10, help="Minimum total count per gene")
...
def write_contrast_counts(source: Path, output: Path, samples: list[str]) -> None:
    with source.open(newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        if reader.fieldnames is None:
            raise ValueError(f"Count matrix is empty: {source}")
        missing = set(samples) - set(reader.fieldnames)
        if missing:
            raise ValueError(f"Count matrix lacks contrast samples: {sorted(missing)}")
        gene_id_col = "Geneid"
        columns = [gene_id_col] + samples
...
def run_ready_contrast(
    ...
    write_contrast_counts(Path(args.gene_counts), counts, samples)
...
def write_done(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    if failed:
        ...
        raise RuntimeError(f"DESeq2 failed for contrast(s): {failed_ids}")
...
def main() -> int:
    ...
    if not plan_rows:
        raise ValueError("Gene differential plan has no rows")
    ...
    if ready_rows:
        ...
        output_rows.extend(
            run_ready_contrast(row, sample_columns, sample_rows, args, rscript)
