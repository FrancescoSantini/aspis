...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-count", type=int, default=10, help="Minimum total count per transcript")
...
def run_ready_contrast(
    ...
    write_contrast_counts(Path(args.transcript_counts), counts, samples)
...
def write_done(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    if failed:
        ...
        raise RuntimeError(f"Transcript DESeq2 failed for contrast(s): {failed_ids}")
...
def main() -> int:
    ...
    if not plan_rows:
        raise ValueError("Transcript differential plan has no rows")
