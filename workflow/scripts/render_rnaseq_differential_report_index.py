    ...
    for row in sorted(method_rows, key=lambda item: (item.get("contrast_id", ""), item.get("method", ""))):
        ...
        links = compact_link_list(
            [
                ...
                (feature_plot_label, plot_row.get("feature_plot", "")),
...
"""
  ...
  <section class="dtu-summary">
    ...
    <p class="note">DTU plots use a shared report structure, but the biological unit differs by method: DRIMSeq/DEXSeq show transcript-usage features, DEXSeqExon shows flattened exon bins, and SUPPA2/rMATS show splicing events. Every completed method has a significance overview and a top genes detail plot. Methods with feature-level statistics also have a ranked candidate plot comparing individual transcript features, exon bins, or splicing events across genes. DRIMSeq significance is gene-level in this output, so its companion detail plot shows transcript-usage proportions for top genes without a separate ranked feature-candidate panel. Delta PSI is the event inclusion shift from control to test. rMATS event codes: SE skipped exon; RI retained intron; A5SS/A3SS alternative splice sites; MXE mutually exclusive exons.</p>
...
def main() -> int:
    ...
    for row in dtu_plot_rows:
        ...
        for label, path_text in [
            ...
            ("feature_plot", row.get("feature_plot", "")),
