...
PROJECT_LAYER_TITLES = {
    "rnaseq_de": "RNA-seq differential expression",
    "enrichment": "GO/Reactome enrichment",
    "dtu_splicing": "Independent DTU/splicing methods",
    "isoform_switch": "Isoform-switch candidates with DTU/splicing support",
    "smallrna_de": "smallRNA differential expression",
    "mirna_targets": "miRNA targets and target feature sets",
    "matched_mirna_mrna": "Matched miRNA-mRNA evidence",
}
...
def project_layer_root(project):
    return f"{PROJECT_REPORT_DIR}/{project}/layers"


def project_layer_html(project, layer):
    return f"{project_layer_root(project)}/{layer}/index.html"


def project_layer_pdf(project, layer):
    return f"{project_layer_root(project)}/{layer}/technical_report.pdf"


def project_layer_pdf_done(project, layer):
    return f"{project_layer_root(project)}/{layer}/technical_report.done"


def project_layer_pdf_qa(project, layer):
    return f"{project_layer_root(project)}/{layer}/technical_report.qa.tsv"


def project_layer_source_summary(project, layer):
    return f"{project_layer_root(project)}/{layer}/source_summary_manifest.tsv"


def project_layer_source_assets(project, layer):
    return f"{project_layer_root(project)}/{layer}/source_asset_manifest.tsv"


def project_layer_manifest(project):
    return f"{project_layer_root(project)}/layer_manifest.tsv"


def project_layers_done(project):
    return f"{project_layer_root(project)}/layers.done"
    ...
    for project in planned_ready_projects(wildcards):
        targets.extend(
            [
                ...
                project_layer_manifest(project),
                project_layers_done(project),
            ]
        )
        for layer in PROJECT_LAYER_KEYS:
            targets.extend(
                [
                    project_layer_html(project, layer),
                    project_layer_pdf(project, layer),
                    project_layer_pdf_done(project, layer),
                    project_layer_pdf_qa(project, layer),
                ]
            )
    return targets


def project_layer_source_inputs(wildcards):
    plan_path = checkpoints.build_analysis_plan.get().output[0]
    inputs = [plan_path]
    with open(plan_path, newline="") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        for row in reader:
            ...
            if assay not in {"rnaseq", "smallrna"}:
                continue
            base = f"{BRANCH_DIR}/{assay}/{wildcards.project}"
            inputs.extend([f"{base}/samples.tsv", f"{base}/design.tsv", f"{base}/fastq_inspection.tsv"])
            if assay == "rnaseq":
                if RNASEQ_DIFFERENTIAL_REPORTS_ENABLED:
                    inputs.extend(
                        [
                ...
                if RNASEQ_ISOFORM_SWITCH_REPORTS_ENABLED:
                    inputs.extend(
                        [
                            f"{base}/differential/isoform_switch/report/switch_event_summary.tsv",
                            f"{base}/differential/isoform_switch/report/switch_candidates.tsv",
                            f"{base}/differential/isoform_switch/report/isoform_interpretation_consensus.tsv",
                            f"{base}/differential/isoform_switch/report/switch_plots.pdf",
                        ]
                    )
                ...
                if SMALLRNA_TARGET_ENRICHMENT_RUN:
                    inputs.append(f"{small}/differential/target_enrichment/target_manifest.tsv")
                if SMALLRNA_TARGET_FEATURE_SET_RUN:
                    inputs.append(f"{small}/differential/target_feature_sets/target_feature_set_manifest.tsv")
                if SMALLRNA_MIRNA_FEATURE_SET_RUN:
                    inputs.append(smallrna_mirna_feature_set_manifest(wildcards.project))
                if MIRNA_MRNA_INTEGRATION_RUN:
                    inputs.append(f"{small}/differential/mirna_mrna_integration/mirna_mrna_manifest.tsv")
                if MIRNA_MRNA_TARGET_FEATURE_SET_RUN:
                    inputs.append(smallrna_mirna_mrna_target_feature_set_manifest(wildcards.project))
    return inputs
...
def project_report_inputs(wildcards):
    inputs = [
        ANALYSIS_PLAN,
        project_layer_manifest(wildcards.project),
        project_layers_done(wildcards.project),
    ]
    for layer in PROJECT_LAYER_KEYS:
        inputs.extend(
            [
                project_layer_html(wildcards.project, layer),
                project_layer_pdf(wildcards.project, layer),
                project_layer_pdf_qa(wildcards.project, layer),
            ]
        )
    return inputs
...
rule render_project_layer_reports:
    input:
        project_layer_source_inputs
    output:
        html=expand(f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/index.html", layer=PROJECT_LAYER_KEYS, allow_missing=True),
        summaries=expand(f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/source_summary_manifest.tsv", layer=PROJECT_LAYER_KEYS, allow_missing=True),
        assets=expand(f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/source_asset_manifest.tsv", layer=PROJECT_LAYER_KEYS, allow_missing=True),
        manifest=f"{PROJECT_REPORT_DIR}/{{project}}/layers/layer_manifest.tsv",
        done=f"{PROJECT_REPORT_DIR}/{{project}}/layers/layers.done"
    params:
        branch_dir=BRANCH_DIR,
        output_root=lambda wildcards: project_layer_root(wildcards.project)
    resources:
        runtime=30,
        mem_mb=4000,
        disk_mb=10000
    log:
        "logs/projects/{project}.layer_reports.log"
    shell:
        r"""
        mkdir -p logs/projects
        python3 workflow/scripts/render_project_layer_reports.py \
...
rule render_project_layer_pdf:
    input:
        html=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/index.html",
        summary=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/source_summary_manifest.tsv",
        assets=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/source_asset_manifest.tsv"
    output:
        pdf=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/technical_report.pdf",
        done=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/technical_report.done",
        qa=f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/technical_report.qa.tsv"
    params:
        title=lambda wildcards: PROJECT_LAYER_TITLES[wildcards.layer],
        summary_option=lambda wildcards: "--smallrna-summary-manifest" if wildcards.layer == "smallrna_de" else "--rnaseq-summary-manifest",
        asset_option=lambda wildcards: "--smallrna-asset-manifest" if wildcards.layer in {"smallrna_de", "mirna_targets", "matched_mirna_mrna"} else "--rnaseq-asset-manifest"
    wildcard_constraints:
        layer="rnaseq_de|enrichment|dtu_splicing|isoform_switch|smallrna_de|mirna_targets|matched_mirna_mrna"
    resources:
        runtime=120,
        mem_mb=16000,
        disk_mb=50000
    log:
        "logs/projects/{project}.{layer}.technical_pdf.log"
    shell:
        r"""
        mkdir -p logs/projects
        python3 workflow/scripts/render_technical_pdf_report.py \
          --project {wildcards.project:q} \
          --project-html {input.html:q} \
          --layer-key {wildcards.layer:q} \
          --layer-title {params.title:q} \
          {params.summary_option} {input.summary:q} \
          {params.asset_option} {input.assets:q} \
          --output {output.pdf:q} \
          --done {output.done:q} \
          > {log:q} 2>&1
...
rule render_project_report_index:
    ...
    params:
        analysis_plan=ANALYSIS_PLAN,
        branch_dir=BRANCH_DIR,
        technical_pdf=lambda wildcards: project_report_pdf(wildcards.project)
    resources:
        runtime=180,
        mem_mb=32000,
        disk_mb=100000
    ...
    shell:
        ...
        python3 workflow/scripts/render_project_report_index.py \
          ...
          --analysis-plan {params.analysis_plan:q} \
          --branch-dir {params.branch_dir:q} \
          --technical-pdf {params.technical_pdf:q} \
          --output {output.html:q} \
          --done {output.done:q} \
          > {log:q} 2>&1
        """
...
rule merge_project_technical_pdf:
    input:
        manifest=f"{PROJECT_REPORT_DIR}/{{project}}/layers/layer_manifest.tsv",
        pdfs=expand(f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/technical_report.pdf", layer=PROJECT_LAYER_KEYS, allow_missing=True),
        qa=expand(f"{PROJECT_REPORT_DIR}/{{project}}/layers/{{layer}}/technical_report.qa.tsv", layer=PROJECT_LAYER_KEYS, allow_missing=True)
    output:
        pdf=f"{PROJECT_REPORT_DIR}/{{project}}/technical_report.pdf",
        done=f"{PROJECT_REPORT_DIR}/{{project}}/technical_report.done",
        qa=f"{PROJECT_REPORT_DIR}/{{project}}/technical_report.qa.tsv"
    resources:
        runtime=60,
        mem_mb=8000,
        disk_mb=100000
    log:
        "logs/projects/{project}.combined_technical_pdf.log"
    shell:
        r"""
        mkdir -p logs/projects
        python3 workflow/scripts/merge_project_layer_pdfs.py \
          --project {wildcards.project:q} \
          --layer-manifest {input.manifest:q} \
          --output {output.pdf:q} \
          --done {output.done:q} \
          > {log:q} 2>&1
        python3 workflow/scripts/validate_technical_pdf_report.py \
