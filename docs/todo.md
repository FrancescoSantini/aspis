...
- Make `workflow/scripts/prepare_feature_set_resources.py` produce a complete
  production resource bundle:
  - gene-set GMT or normalized TSV outputs;
  - transcript-to-gene mapping where needed;
  - gene symbol, Ensembl gene, Entrez, and transcript ID mapping reports;
  - `unmapped_features.tsv`;
  - resource manifest with provenance;
  - config fragment that can be pasted into a project YAML.
- Make `workflow/scripts/prepare_mirna_target_resources.py` produce a complete
  target bundle:
...
- Add or improve preflight checks for resource bundles:
  - required columns;
  - non-empty files;
  - duplicate term-feature pairs;
  - unsupported namespaces;
  - insufficient mapping against the actual result tables;
  - license/status metadata presence.
