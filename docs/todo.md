...
- Decide and document the default open resource set for public examples and
  BEAS validation. Candidate categories:
  - GO biological process, molecular function, and cellular component gene sets
    from open GO sources.
  - Reactome pathway gene sets from open Reactome exports.
  - Project-owned or explicitly open GMT/TSV custom sets.
  - Open miRNA target tables only after license review and column validation.
  - Optional open protein/domain/consequence tables, for example InterProScan
    TSVs generated by the user or open Pfam/HMMER-derived outputs where the
    local installation and database license permit use.
