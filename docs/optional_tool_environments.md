...
- `rMATS` or `rMATS-turbo`, because installations often vary by Python,
  compiler, and cluster setup. ASPIS can run rMATS natively once `rmats.py`
  and the run-specific read length are configured.

Native DRIMSeq/DEXSeq/DEXSeqExon/SUPPA2/rMATS DTU is configured under:
...
`dexseq_count.py`, `suppa.py`, or `rmats.py` is missing, the DTU manifest records a blocked
...
ASPIS; local `ioe` event types are a future extension. Native rMATS writes
control/test BAM lists per contrast, runs junction-event testing from aligned
BAMs plus the configured GTF, and standardizes `*.MATS.JC.txt` or
`*.MATS.JCEC.txt` event tables. `rmats_read_length` must be set explicitly;
the default `0` blocks rMATS with a clear manifest reason.
