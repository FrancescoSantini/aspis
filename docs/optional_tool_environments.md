...
Native DRIMSeq/DEXSeq/SUPPA2 DTU is configured under:
...
If `Rscript`, `R::DRIMSeq`, `R::DEXSeq`, or `suppa.py` is missing, the DTU
manifest records a blocked status. Native DEXSeq currently runs transcript
features grouped by gene; true exon-bin DEXSeq still needs an exon-count layer.
Native SUPPA2 currently runs transcript-event `ioi` mode from transcript
expression files generated by ASPIS; local `ioe` event types are a future
extension. rMATS is still available only as a command-template method while its
native event/count input contract remains undecided.
